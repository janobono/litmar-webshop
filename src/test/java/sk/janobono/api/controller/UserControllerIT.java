...
import sk.janobono.api.service.so.UserDataSO;
import sk.janobono.api.service.so.UserProfileSO;
...
class UserControllerIT extends BaseIntegrationTest {
    ...
    private UserSO getUser(UUID id) throws Exception {
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.get("/api/users/{0}", id)).andReturn();
    ...
    private Page<UserSO> getUsers(Pageable pageable) throws Exception {
        ...
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.get(
                "/api/users?page={0}&size={1}&sort={2}", params.get("page"), params.get("size"), params.get("sort"))).andReturn();
        assertThat(mvcResult.getResponse().getStatus()).isEqualTo(HttpStatus.OK.value());
    ...
    private Page<UserSO> getUsers(String searchField, String username, String email, Pageable pageable) throws Exception {
        Map<String, Object> params = pageableToParams(pageable);
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.get(
                "/api/users/by-search-criteria?search-field={0}&username={1}&email={2}&page={3}&size={4}&sort={5}",
    ...
    private void setUser(UserSO userSO) throws Exception {
        ...
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.put("/api/users/{0}", userSO.id())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    private void setAuthorities(UserSO userSO) throws Exception {
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.patch("/api/users/{0}/authorities", userSO.id())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    private void setConfirmed(UserSO userSO) throws Exception {
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.patch("/api/users/{0}/confirm", userSO.id())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    private void setEnabled(UserSO userSO) throws Exception {
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.patch("/api/users/{0}/enable", userSO.id())
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapToJson(Boolean.TRUE))).andReturn();
        assertThat(mvcResult.getResponse().getStatus()).isEqualTo(HttpStatus.OK.value());
        UserSO result = mapFromJson(mvcResult.getResponse().getContentAsString(), UserSO.class);
    ...
    private void deleteUser(UUID id) throws Exception {
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.delete("/api/users/{0}", id)).andReturn();
        assertThat(mvcResult.getResponse().getStatus()).isEqualTo(HttpStatus.OK.value());
        mvcResult = mvc.perform(MockMvcRequestBuilders.get("/api/users/{0}", id)).andReturn();
